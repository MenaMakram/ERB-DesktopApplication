...
namespace ProjectsERB
{
    ...
    public partial class ProductReturnBack : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            var query = from c in Our.invoice_products
                        where c.SerialNumber == serial && (c.Invoice.InvoiceType_ID == 1|| c.Invoice.InvoiceType_ID == 6) && (c.Invoice.Person.PersonType_ID==1|| c.Invoice.Person.PersonType_ID == 3)
                        select new {
                            invoiceNumber =c.Invoice.ID,
                            personName=c.Invoice.Person.Name,
                            c.Invoice.Person.Phone,
        ...
        private void dataGridView1_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if (dataGridView1.Rows[index].Cells["TypeName"].Value.ToString() == "مرتجع التاجر للمخزن") {
                label5.Visible = true;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (textBox1.Text != "" && textBox2.Text != "" && textBox2.Text != "0" && dataGridView1.SelectedRows.Count > 0)
            {
                ...
                var prodin3 = Our.invoice_products.Where(iv => iv.SerialNumber == textBox1.Text && iv.Product_ID == prodID && iv.Invoice.InvoiceType_ID == 6).Sum(vs => (int?)vs.Quantity) ?? 0;
                ...
                var totalfound = ((prodin + prodin2+prodin3) - (prodout2 + prodout1));
                ...
                if (totalfound >= quantity || totalfound >= quantity)
                {
                    ...
                    if (dataGridView1.Rows[index].Cells["TypeName"].Value.ToString() == "مرتجع التاجر للمخزن")
                    {
                        int id = int.Parse(comboBox3.SelectedValue.ToString());
                         ...
                         personDetails = (from person in Our.person
                                                where person.ID == id && person.PersonType_ID == 1
                   ...
                   personDetails.AccountMoney -= x.TotalCash;
        ...
        private void getmorade()
        {
            comboBox3.DisplayMember = "Name";
            comboBox3.ValueMember = "ID";
            comboBox3.DataSource = Our.person.Where(pa=>pa.PersonType_ID==1).ToList();
        }
