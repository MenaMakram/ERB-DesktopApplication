...
namespace ProjectsERB
{
    public partial class AddCategory : Form
    {
        ...
        private void buttonDelete_Click(object sender, EventArgs e)
        {
            ...
            var products = context.Products.Where(pr => pr.categories.ID == quer.ID).ToList();
            foreach (var item in products)
            {
                var ivpds = context.invoice_products.Where(uv => uv.Products.ID == item.ID).ToList();
                context.invoice_products.RemoveRange(ivpds);
            }
